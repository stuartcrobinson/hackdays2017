...
all_interactions = load_sparse_coo("interactionsAll_sparse_coo_matrix.npz")
...
cfModelFile = 'filename.pkl'


start_time2 = time.time()

if os.path.isfile(cfModelFile):
    model = joblib.load(cfModelFile) 
    print("Loading model took %g seconds" % (time.time() - start_time2))
    ...
    print("Fitting model took %g seconds" % (time.time() - start_time2))
...
hybridModelFile = 'hybrid_model.pkl'

if False and os.path.isfile(hybridModelFile):
    model = joblib.load(hybridModelFile)  
    print("Loading model took %g seconds" % (time.time() - start_time2))
...
#     model = model.fit(train, user_features=all_interactions.tocsr(), item_features=item_features, epochs=NUM_EPOCHS, num_threads=NUM_THREADS)  # --> ValueError: The user feature matrix specifies more features than there are estimated feature embeddings: 1330 vs 42937.
...
joblib.dump(model, hybridModelFile) 
